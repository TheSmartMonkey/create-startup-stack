...
import { BouncedDb } from '@libs/db/bounced/bounced.db';
...
export async function main(event: SQSEvent, context: Context): Promise<void> {
  try {
    ...
    const mailgunEvents = mailgun.filterMessagesFromSQS(uniqueMessages, MailgunEntityEnum.BOUNCED);
    ...
    const noneProcessed = await new BouncedDb().insertMany(verifiedMailgunEvents);
    ...
    const dlqName = process.env.ADD_TO_BOUNCED_QUEUE_DLQ_NAME;
